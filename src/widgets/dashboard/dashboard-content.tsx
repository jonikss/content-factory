    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-bg2 border border-border rounded-lg px-3.5 py-3">
              <div className="text-[10px] text-text3 uppercase tracking-wider mb-1">
                {stat.label}
              </div>
              <div className={`text-xl font-semibold font-mono tracking-tight ${stat.color}`}>
                {stat.value}
              </div>
            </div>
          ))}
