...
const seoSchema = z.object({
  ...
  meta_description: z.string().max(155).describe('Meta description, max 155 chars'),
  slug: z.string().regex(/^[a-z0-9-]+$/).describe('URL slug, lowercase with hyphens'),
  focus_keyword: z.string().describe('Primary focus keyword'),
  image_alts: z.array(z.string()).max(5).describe('Alt texts for potential images'),
})
