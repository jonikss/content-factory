...
const qualitySchema = z.object({
  score_relevance: z.number().min(0).max(100).describe('How relevant the article is to the keyword (0-100)'),
  score_water: z.number().min(0).max(100).describe('Content density score — higher means less filler (0-100)'),
  score_structure: z.number().min(0).max(100).describe('Structure and readability score (0-100)'),
  total: z.number().min(0).max(100).describe('Overall quality score (0-100)'),
  decision: z.enum(['publish', 'rewrite', 'reject']).describe('Decision based on total score'),
  issues: z.array(z.string()).describe('List of specific issues found'),
  rewrite_hint: z.string().nullable().describe('Specific hint for improvement if rewrite is needed'),
})
