...
function getStageStatuses(events: StreamEvent[]): Record<string, { status: StageStatus; message: string }> {
...
export function StreamProgress({ events, sectionCount, totalSections }: StreamProgressProps) {
  ...
  return (
    <div className="flex flex-col">
      {stages.map((stage) => {
        ...
        return (
          <div key={stage.key} className="flex items-start gap-2.5 py-2 border-b border-border last:border-b-0">
            {/* Icon */}
            <div
            ...
            <div>
              <div
                className={`text-xs font-medium ${
                  status === 'active' || status === 'done' ? 'text-text' : 'text-text2'
                }`}
              >
                {stage.label}
              </div>
