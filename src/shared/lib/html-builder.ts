  ...
  for (const block of blocks) {
    ...
    if (lines.every((l) => /^[-*]\s/.test(l))) {
      const items = lines.map(
        (l) =>
          `    <li>${convertInlineFormatting(l.replace(/^[-*]\s+/, ""))}</li>`,
      );
      htmlParts.push(`  <ul>\n${items.join("\n")}\n  </ul>`);
      continue;
    }
    ...
    if (lines.every((l) => /^\d+\.\s/.test(l))) {
      const items = lines.map(
        (l) =>
          `    <li>${convertInlineFormatting(l.replace(/^\d+\.\s+/, ""))}</li>`,
      );
      htmlParts.push(`  <ol>\n${items.join("\n")}\n  </ol>`);
      continue;
    }
    ...
    // Regular paragraph
    htmlParts.push(
      `  <p>${convertInlineFormatting(trimmed.replace(/\n/g, " "))}</p>`,
    );
  }
...
export function buildHtml(
  ...
  // Intro
  parts.push(
    `  <p class="intro">${convertInlineFormatting(content.intro.replace(/\n{2,}/g, '</p>\n  <p class="intro">'))}</p>`,
  );
  parts.push("");
  ...
  parts.push(
    `    <p>${convertInlineFormatting(content.conclusion.replace(/\n{2,}/g, "</p>\n    <p>"))}</p>`,
  );
  parts.push("  </section>");
  parts.push("");
