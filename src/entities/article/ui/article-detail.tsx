...
export function ArticleDetail({ article }: ArticleDetailProps) {
  ...
  return (
    <div
      className="grid grid-cols-[1fr_200px] gap-3"
    ...
    >
              ...
              {content.sections.map((section, i) => (
                <div key={i} className="mb-3">
                  ...
                  {section.body.split(/\n{2,}/).map((p, j) => (
                    <div
                      key={j}
                      className="text-[11px] text-text2 leading-relaxed mb-1"
                    >
                      {p}
                    </div>
                  ))}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="bg-bg2 border border-border rounded-lg px-3 py-3">
          ...
          <div
            className={`text-[28px] font-semibold font-mono tracking-tight ${scoreColor}`}
          >
          ...
          {[
            {
              ...
              value:
                (
                  article.content_json as { word_count?: number } | null
                )?.word_count?.toLocaleString("ru-RU") ?? "—",
            },
            {
              label: "Разделов H2",
              value:
                (article.content_json as { sections?: unknown[] } | null)
                  ?.sections?.length ?? "—",
            },
            {
              label: "Создана",
